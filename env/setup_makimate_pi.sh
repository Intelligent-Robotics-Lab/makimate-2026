...
#
# Some packages are pre-installed by apt without a pip RECORD file, so pip
# can't uninstall them when upgrading.  Reinstall them via pip first (with
# --ignore-installed to skip the uninstall step) so a proper RECORD file
# exists, then install everything else.
